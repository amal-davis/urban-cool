...
import cardAc from '../assets/photos/hero-ac.webp'
import cardRefrigerator from '../assets/photos/hero-refrigerator.webp'
import cardWashingMachine from '../assets/photos/hero-washing-machine.webp'
import cardMicrowave from '../assets/photos/hero-microwave.webp'
...
export interface Service {
  ...
  ariaLabel: string
  /** Small-badge illustration — the Book a Service tile's corner badge, My
   *  Bookings cards, and the Tracking page's booking summary, all contexts
   *  too small for a real photo to read well. Also `imageUrl`'s fallback
   *  wherever a photo isn't set. Optional: services fetched from the
   *  backend (see lib/servicesApi.ts) always carry a real `imageUrl`
   *  instead, so they never set this — only the icon-based contexts above
   *  (which stay sourced from the hardcoded list below) actually need it. */
  ...
  imageUrl?: string
  /** Book a Service tile's large corner illustration — an isolated product
   *  render on a transparent background (see assets/photos/CREDITS.md),
   *  not `imageUrl`: that's a full lifestyle photo with a busy background
   *  (a wall, a kitchen), which reads as clutter floating in a tile corner
   *  rather than a clean product shot — the same reasoning ServiceCard.css
   *  already documented when the tile redesign first shipped with the
   *  Icon-as-illustration fallback these renders now replace. Optional for
   *  the same reason `Icon` is: backend-fetched services never set it. */
  cardImage?: string
